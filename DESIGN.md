...
*(2026-08-22: closed the remaining gaps — the services grid and case-studies list now have scroll-reveal (both
were previously un-animated), the hero background video now actually honors `prefers-reduced-motion` instead of
always autoplaying, and the service-card spotlight's `--spot-x`/`--spot-y` custom properties are now set via a
`pointermove` listener instead of sitting static at their `50%` default.)*
...
| `/portfolio` | — (redirect in `astro.config.mjs`) | redirect-only → `/services`, nav item intentionally commented out |
| `/404` | `src/pages/404.astro` | built (noindex) |

Routes use no trailing slash (`trailingSlash: 'never'` in `astro.config.mjs`, matching the reference site's URL
scheme) — this overrides the generic trailing-slash rule in AGENTS.md for this project specifically.

*(Corrected 2026-08-22 — previously listed `/industries/`, `/insights/`, and a standalone `/maintenance/` page as
built; none exist in `src/pages` or in `NAV_ITEMS`.)*
